...
exports.learningHubGenerate = (0, https_1.onCall)({
    ...
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "User must be authenticated to use Learning Hub");
    }
    const { userInput } = request.data;
    if (!userInput || typeof userInput !== 'string' || userInput.trim().length === 0) {
        throw new https_1.HttpsError("invalid-argument", "User input is required and must be a non-empty string");
    }
    try {
...
IMPORTANT: Keep ALL text fields SHORT to ensure complete JSON. Return ONLY the JSON, nothing else.`;
        ...
        if (errorMessage.includes('503') || errorMessage.includes('overloaded')) {
            throw new https_1.HttpsError("resource-exhausted", "AI service is currently busy. Please try again in a few moments.");
        }
        if (errorMessage.includes('JSON')) {
            throw new https_1.HttpsError("internal", "Failed to parse AI response. Please try a shorter or simpler topic.");
        }
        throw new https_1.HttpsError("internal", "Failed to generate learning content. Please try again.");
    }
});
//# sourceMappingURL=learningHubGenerate.js.map